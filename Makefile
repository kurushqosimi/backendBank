postgres:
	docker run --name postgres --network bank-network -p 5433:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=secret -d postgres:17.2-alpine3.21
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/kurushqosimi/backendBank/db/sqlc Store
